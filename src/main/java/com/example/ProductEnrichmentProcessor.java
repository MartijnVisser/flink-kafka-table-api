...
import com.example.lookup.MySqlAsyncScalarFunction;
...
public class ProductEnrichmentProcessor {
  ...
  protected void registerLookupFunction(TableEnvironment tableEnv) {
    // Register separate functions for each field
    tableEnv.createTemporaryFunction(
        "lookup_name",
        new MySqlAsyncScalarFunction(
            mysqlHost,
            mysqlPort,
            mysqlDatabase,
            mysqlUsername,
    ...
    LOG.info("Registered async lookup functions");
  }
    ...
    Table enrichedProducts =
        products
            ...
            .select(
                ...
                $("quantity"),
                $("timestamp"),
                call("lookup_name", $("source_product_id")).as("name"),
                call("lookup_brand", $("source_product_id")).as("brand"),
                call("lookup_vendor", $("source_product_id")).as("vendor"),
                call("lookup_department", $("source_product_id")).as("department"));
